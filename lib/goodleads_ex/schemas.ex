defmodule GoodleadsEx.Schemas do
  @moduledoc """
  Generated schema structs from the GoodLeads OpenAPI specification.
  ...
  @all_schemas @openapi_spec["components"]["schemas"]

  for {schema_name, schema_def} <- @all_schemas do
    mod_name = Module.concat(__MODULE__, schema_name)

    # Resolve properties, following allOf $ref when no local properties exist
    props =
      case {schema_def["properties"], schema_def["allOf"]} do
        {local_props, _} when is_map(local_props) and map_size(local_props) > 0 ->
          local_props

        {_, [%{"$ref" => ref} | _]} ->
          ref_name = ref |> String.split("/") |> List.last()
          ref_schema = Map.get(@all_schemas, ref_name, %{})
        ...
        defp convert_field(key, value) do
          case Map.get(@conversions, key) do
            {:struct, ref_name} when is_map(value) ->
              Module.concat(GoodleadsEx.Schemas, ref_name).from_map(value)

            {:list, ref_name} when is_list(value) ->
              mod = Module.concat(GoodleadsEx.Schemas, ref_name)
